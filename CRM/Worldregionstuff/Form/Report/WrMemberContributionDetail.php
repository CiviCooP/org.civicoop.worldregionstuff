 ...
 * @copyright CiviCRM LLC (c) 2004-2017
 */
class CRM_Worldregionstuff_Form_Report_WrMemberContributionDetail extends CRM_Report_Form {
...
";
    ...
    if ($this->_addressField OR
      (!empty($this->_params['state_province_id_value']) OR
        !empty($this->_params['country_id_value']) OR
        !empty($this->_params['fields']['region_id']) OR
        !empty($this->_params['region_id_value']))
    ) {
      $this->_from .= "
    ...
    // add worldregion froms if required
    if (!empty($this->_params['fields']['region_id'])) {
      $this->_from .= "
  ...
  public function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      if (array_key_exists('civicrm_country_region_id', $row)) {
        if ($value = $row['civicrm_country_region_id']) {
          $rows[$rowNum]['civicrm_country_region_id'] = CRM_Core_PseudoConstant::worldRegion($value);
        }
        $entryFound = TRUE;
      }
